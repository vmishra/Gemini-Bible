...
for the full per-knob rationale. Structured-output deviations:
  ...
  • response_mime_type="application/json" + response_json_schema=...
    ...
    Field descriptions on the Pydantic model are forwarded to the model
    as per-field guidance.

  • thinking_level="medium" (Gemini 3.x default is "high")
    Same rationale as text/basic. JSON tasks rarely need "high" since
    schema constraints already prune the search space.
"""
